...
| "The backbone ablation vindicates V-B" (unqualified) | "Deep beats near-linear-wide **at equal recipe**" — headline permitted, on two conditions: every occurrence names the axis (depth / shallow--wide, identical recipe), and none may read as "V-B is the right design", because the ground it was actually chosen on (throughput) is untested here | A ~204 k-param dense head on 25 500 flattened features is a near-linear wide model; and the ground V-B was actually chosen on (throughput) is untested here. **Amended 2026-07-26** — the rule previously ended "appendix ablation, never a headline". Two reasons that clause fell: a 6-page conference paper has no appendix, so it was unsatisfiable by construction; and C1 vs C1_sharplike is one of only **two** comparisons the bootstrap resolves, the one with the wider margin, and it survives jackknife removal of any single test trace. Relegating the study's most robust measurement was the wrong trade — the load-bearing half of the rule was always the qualifier, not the placement, and the qualifier now binds *every* occurrence instead of one. |
