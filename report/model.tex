...
same front-end, windowing, encoder, fusion rule and harness, so a
...
fusion, anchoring our harness against the prior approach rather than entering
the objective comparison (\secref{sec:res_baseline}).
...
antennas, worth $8.5$ test points over the best single antenna.}
\label{fig:pipeline}
\end{figure}
...
inventory counts $101$ traces and $404$ per-antenna streams. Activities carry
the dataset's letter codes: the five-class core is E (empty room), W (walking),
R (running), J (jumping) and L (sitting still), and the extended set adds C
(sit-down and stand-up), H (arm exercise) and S (standing still).

\textbf{Front-end, windowing and normalization.} The data is distributed as
Doppler traces already produced by the reference front-end~\cite{Meneghello-2023} (CSI phase
...
reflected power over $100$ velocity bins and time, which we consume as-is.
Each map is cut into windows of $340$ time steps $\times$ $100$ velocity bins
...
primary rotation trains on $\approx53$k (window, antenna) samples and is
...
\textbf{Augmentation.} On the fly, after standardization, in a fixed order:
circular time shift ($\pm10$ frames), time masking ($1$--$2$ masks of $5$--$20$
frames), velocity masking ($1$--$2$ masks of $2$--$10$ bins) and amplitude
scaling ($s\sim\mathcal{U}(0.8,1.2)$), masks filled with the
post-standardization mean. The cross-entropy configurations apply each with
probability $0.5$; the contrastive one draws \emph{two independent views} of
each window at $0.8$, with additive Gaussian noise ($\sigma=0.05$) at $0.5$,
since SupCon needs views hard enough for its positives to be
informative~\cite{Chen-2020}.
Velocity is masked sparingly,
...
inflates every metric~\cite{Hammerla-2015} --- and splits are frozen once as JSON under version
...
rejected before any run: the bedroom holds five of the seven capture sets, so
its training side would be two single-session environments totalling $26$
traces, with a two-trace validation split --- confounding ``the model does not
generalize'' with ``the model had no data''. That skewed incidence also has a
structural consequence (\secref{sec:res_diag}).
...
\label{eq:grl}
\end{equation}
with $p = \min(\mathrm{epoch}/20, 1)$, the ramp of~\cite{Ganin-2016} fixed in
advance so the adversary reaches full strength independently of when early
stopping fires.
...
loss~\cite{Khosla-2020} at temperature $\tau=0.1$. C3 trains in two phases: phase A optimizes it for a fixed horizon of $60$ epochs with no
...
probe on cached features~\cite{Chen-2020}, and only the grid checkpoint with the best
...
\node[enc, right=4mm of in] (e) {encoder $f_\theta$\\ResNet, adapted\\$d_{\mathrm{enc}}{=}256$};
...
\node[box, below=9mm of e, text width=40mm] (pb)
  {phase B: freeze $f_\theta$, cache features, fit linear probe};
\draw[->, dashed] (sc.south) |- (pb.east);
\draw[->, dashed] (e.south) -- (pb.north);
\end{tikzpicture}
\caption{Learning framework: one encoder, three objectives, tagged at right.
The dashed path doubles as the diagnostic
...
C4 was \emph{closed on evidence and never trained}: with no readable domain
target under either loss family (\secref{sec:res_diag}), its expected outcome
was ``C3 plus noise''. Three pre-registered arms
complete the picture: C3-ft (a full CE fine-tune from the SupCon encoder),
C1-aug (amplitude augmentation) and C1-sharplike (the backbone ablation).
...
trivial positives. Optimization is AdamW
...
fixed steps ($300$ in phase A), at most $40$ CE epochs, early stopping on fused
...
maps a checkpoint to per-set metric files --- accuracy, macro-F1, per-class
F1, confusion matrices and expected calibration error~\cite{Guo-2017} --- always reported with
the
