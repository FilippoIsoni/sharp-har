...
it, so its features do not encode what separates the activities. The GRL loses
$15.8$ points, with a $10\times$ larger seed spread, because it has no target:
no domain attribute is linearly readable from the features an adversary would
consume, a structural consequence of how environments are distributed in the
dataset. The reference design's shallow--wide network loses $23.5$ points in
