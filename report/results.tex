...
$0.5$ accuracy points) and no variant resolves an improvement over it; neither
objective helps, SupCon falling $7.8$ and the GRL $15.8$ points below it; and
the largest single effect in the table is not an objective but the backbone,
worth $23.5$ points at equal recipe.

\begin{table}[t]
\centering
\caption{Main results: validation macro-F1 (used for checkpoint selection),
test macro-F1 and accuracy on the held-out session, and the paired accuracy
...
\emph{behaviour} of the prior approach, which it does, degrading to macro-F1
$0.605$ over the held-out capture sets much as the original work reports.
Because C0 differs from our model in protocol, class set, fusion \emph{and}
architecture at once, the like-for-like comparison is the ablation
\emph{C1-sharplike}: the reference network inside the exact C1 recipe, only
the backbone changed. It loses $23.5$ accuracy points (test macro-F1 $0.543$
vs.\ $0.804$), CI $[-0.399,-0.092]$. Read honestly this says ``deep beats
near-linear-wide in this recipe'', on one axis only; its role is to close the
objection that our CE baseline is a strawman, so that the nulls below are
measured against a strong control.
...
struggles: both encoders form tight activity clusters, but C3's low-motion
classes (L, S and the empty room) chain into one filament instead of three
separable blobs, and the same diagnostic on C3-ft shows the fine-tune
\emph{un-chaining} that geometry --- turning the SupCon encoder back into C1
rather than building on it. Counted honestly this is two families of evidence,
...
is unresolved, so we report a direction with a range, not a point estimate.
The replicates add a second observation: the same-configuration spread is
...
The probe itself only shows that the domain is not \emph{linearly} decodable
from traces the encoder has seen; it is the structural argument that
generalizes the verdict to the dataset.

\begin{figure}[t]
\centering
\includegraphics[width=0.88\columnwidth]{embeddings_c1_vs_c3.pdf}
\caption{t-SNE~\cite{vanderMaaten-2008} of train features, identical recipe;
top row C1 (CE), bottom row C3 (SupCon). \emph{Left, by activity:} both form
tight clusters, but C3's L/S/E clusters chain into one filament instead of
three separable blobs --- the geometric counterpart of its lower linear-probe
score. \emph{Right, the same points by capture set:} colours are uniformly
mixed inside every cluster, in both encoders --- no domain sub-structure
exists for an adversary to remove.}
\label{fig:tsne}
\end{figure}
...
Checkpoint selection was blind to C, E and S on the primary rotation and we
had pre-declared the risk that those classes would underperform; the opposite
is true, the val-blind classes being the \emph{best} (mean F1 $0.904$ against
$0.744$ for the val-seen ones), so class difficulty in the target environment,
not selection blindness, drives per-class error. The errors themselves are
...
physically similar activities indicate that the model transfers genuine motion
features rather than room artifacts. Calibration
