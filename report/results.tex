...
$0.5$ points for C1 against $4.7$ for C2. A linear probe on C2's \emph{frozen}
features reaches $0.720$ test accuracy, $7$ points above its own trained head
($0.647$), against $+0.002$ for C1: the damage splits about evenly
between representation and head.
...
checkpoints and on the second rotation return the same
verdict, the largest positive deviation anywhere being $+0.015$ --- though C2's
own control falls to $0.893$, the adversary's cost in fit, not only transfer; the right
...
encoder is then gradient noise, and \secref{sec:res_grl} locates its damage in both the
representation and the head.
