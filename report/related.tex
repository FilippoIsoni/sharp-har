...
micro-Doppler spectrogram is widely used~\cite{Wang-2019,Wang-2026}. Our reference point is SHARP~\cite{Meneghello-2023}:
...
\textbf{Domain generalization by adversarial invariance.} The best-known
recipe for making a representation domain-invariant is adversarial: a domain
...
head still classifies. WiFi sensing pursues the same invariance
adversarially~\cite{Wang-2021} or by aligning feature distributions across
training environments~\cite{Wang-2024}, on the implicit and rarely tested
...
same-label samples as positives, and reports representations more robust and
transferable than cross-entropy ones --- a transferability since contested for
objectives that improve in-domain accuracy~\cite{Kornblith-2021}.
The promise is geometric ---
...
generalization by meta-learning or self-supervised
pre-training~\cite{Wang-2026}; these are out of scope here, since each replaces
