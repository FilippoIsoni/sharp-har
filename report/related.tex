...
hand-crafted statistics of the amplitude/phase time series to shallow
classifiers; the field then moved to deep models operating on time--frequency
representations, of which the micro-Doppler spectrogram is the most
common~\cite{Wang-2019}. Our reference point is SHARP~\cite{Meneghello-2023}:
...
head still classifies. DANN-style training has been transplanted into CSI
sensing to remove environment- and subject-specific nuisance factors, on the
implicit and rarely tested precondition that the nuisance factor is present
and readable in the features; when it is not, the adversary reduces to a
gradient of noise. A central point of this report is to \emph{measure} that
precondition instead of assuming it.

\textbf{Contrastive representation learning.} SupCon~\cite{Khosla-2020}
extends the self-supervised InfoNCE objective~\cite{Chen-2020} by treating all
...
empirical: we interrogate it with linear, nearest-class-mean and kNN readouts,
a concatenation control and a full fine-tune, so that a negative result cannot
be blamed on the readout. Other lines pursue generalization by
meta-learning~\cite{Ding-2020}, dedicated domain
generalization~\cite{Yang-2024} or self-supervised
...
study avoids. Prior work reports overwhelmingly the cases where such
objectives \emph{helped}; a controlled setting in which they do not, plus a
diagnostic that predicts it, is what we contribute.
