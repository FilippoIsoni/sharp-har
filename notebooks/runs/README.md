...
| `2026-07-16_c2_grl.ipynb` | `c2_grl` | Complete; best val macro-F1 0.8415 @ epoch 13, early stop 23/40. See the corrected §6-C2 monitoring reading in `PROJECT.md` (Part III §6). |
...
| `2026-07-19_c3_ft.ipynb` | `c3_ft` | 13th pre-registered row, clean single session (init log line verified: backbone from `C3/epoch40.ckpt`, head fresh). **Best val macro-F1 0.8183 @ epoch 4** (during warmup, LR 8e-5), early stop 14/40 — val dropped to 0.685 the epoch peak LR hit and never re-passed the best. ≈ C3-lin (0.8190): the pre-registered "comparable to C1 ±1" hypothesis is **falsified** — full-network fine-tuning does not lift the SupCon representation above its own linear-probe ceiling (C1: 0.8871, gap ~6.9 pts). Run launched ahead of the wiring cross-review (declared; the init line + warmup trajectory behaviorally confirm the wiring, formal code review still due pre-freeze). |
| `2026-07-21_c1_ce_aug.ipynb` | `c1_ce_aug` | C1-aug arm, S7-out rotation (`p2_lab`, `ce_amp` profile, seed 42), full 40/40. **Best val macro-F1 0.8830 @ epoch 31**. Paired vs C1 no-aug (0.8871): **Δbest −0.004** (≈ zero, in the §0.5 band), Δmedian +0.028. The augmentation gives NO in-domain val gain here — C1 was already near the val ceiling (dynamic classes {H,J,L,R,W}). Contrast the S6-out row below (+0.147, low baseline, static classes): the two together show a **regularizer that helps where there is headroom, not a cross-domain lever** — cross-domain verdict is the paired Δ on the S7 test, session-only. |
| `2026-07-21_c1_s6out_aug.ipynb` | `c1_ce_s6out_aug` | C1-aug arm, S6-out rotation (`p2_living`, `ce_amp` profile, seed 42), full 40/40. **Best val macro-F1 0.9230 @ epoch 37** (cosine tail, like C1). Paired vs the no-aug S6-out baseline (0.7761): Δbest +0.147, **Δmedian-plateau +0.186** (the max-selection-robust figure). The val gain is real and robust — but this is **in-domain val** (p2_living val is bedroom+lab, the held-out env is S6=living); the cross-domain thesis is decided only by the paired Δ on the **S6 test**, in the single session. An in-domain gain is equally consistent with generic small-data regularization; notebook 06 distinguishes (Δtest ≈ Δval → regularization; Δtest ≫ Δval → cross-domain lever). n=1 rotation / n=1 seed / 6-trace val — magnitude may shrink on test. |
| `2026-07-21_c1_sharplike.ipynb` | `c1_sharplike` | Backbone ablation: SHARP-paper `sharp_like` in the exact C1 recipe on p2_lab (only the backbone differs from C1). **Best val macro-F1 0.7384 @ epoch 24**, early stop 34/40 — **~15 pts below C1 (0.8871)**, median plateau 0.672 vs 0.759 (not a fluke). The deep V-B net decisively beats the paper's near-linear shallow-wide net → V-B choice vindicated on accuracy (already chosen on throughput; sharp_like is ~2.8× faster, 0.191 vs 0.53 s/step). Promoted to the 17th §0.7 test row (2026-07-21, pre-register-and-commit-to-report, expected-confirmatory; the list closed at 16 rows when L6 dropped `C1_aug_s43` on 2026-07-22). Caveat: measures "deep vs near-linear", not only the paper's design. |
