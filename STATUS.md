...
- **Backbone ablation `C1_sharplike` — team-decided + implemented (2026-07-21;
  ...
  - **RESULT (run complete 2026-07-21, archived `notebooks/runs/2026-07-21_c1_sharplike.ipynb`):**
    **best val macro-F1 0.7384 @ epoch 24**, early stop 34/40. **≈15 pts below C1
    (0.8871)** — far outside the §0.5 band and the 0.87-pt C1 seed floor; median
    plateau 0.672 vs C1's 0.759 (−9 pts) confirms it is not a selection fluke.
    The deep V-B net decisively beats the paper's near-linear shallow-wide
    architecture in the C1 recipe → **the V-B choice is vindicated on accuracy**
    (and it was already chosen on throughput). Throughput here: **0.191 s/step vs
    V-B's ~0.53** — sharp_like is ~2.8× faster, so the real trade-off is
    accuracy-vs-speed and V-B wins accuracy by a wide margin. Interpretation
    caveat stands: this is "deep beats near-linear", partly expected; frame as an
    appendix ablation, not a headline. sharp_like was adequate in the paper's own
    P1/5-class setup; it does not carry that adequacy to the harder LOEO/8-class
    recipe.
  - **§0.7 TEST row — DECIDED 2026-07-21 (team): C1_sharplike EARNS a test row
    (17th).** Governance: the decision was taken AFTER seeing val (0.7384) — which
    is admissible because val ≠ test, and there is **no cherry-pick incentive**
    (val already shows it clearly worse, so the row is committed to reporting an
    *expected-confirmatory* result, not fishing for a good number). Registered as
    **pre-register-AND-commit-to-report** (reported whatever S7 gives). The ~15-pt
    val gap is a LARGE concordant gap → survives the L2 multiplicity concern (it
    is exactly the kind of row that does). §0.7 list 16 → 17, §8.4 +0 (no new run
...
8. **Post-session analysis — DONE + RE-EXECUTED 2026-07-23** (`NOTEBOOK_06_REVIEW.md`):
   ...
   **Audit 2026-07-20 (`CONSOLIDATION_REVIEW.md` §6): the template is sound but not
   ...
   contact.**
9. **Report — REFINED + 6-PAGE BUILD VERIFIED 2026-07-23** (accuracy/honesty/
   style pass over the whole `report/` draft, numbers re-checked against
   `report/tables/` + `reports/final/`): authors resolved (Isoni, Schiabel,
   Dedej; `\tbd` macro removed); accuracy fixes — C0 test described as held-out
   capture sets (same bedroom on later campaigns + the two unseen rooms), NOT
   "unseen environments"; val(5-cls)→test(8-cls) scale-crossing claims removed
   (incl. the 15→24 "gap grows" reading); "highest of all configurations"
   corrected (C1+T3A is higher, unresolved); NCM/kNN quoted as macro-F1
   consistently; AdaBN delta shown with its zero-crossing CI; GRL instability
   worded per L7 (n=2 observed range); probe-replication scope = two rotations /
   three loss families / five encoders (not "two seeds"); SHARP related-work
   description made fair (it targets env-independence). Style — single-paragraph
   abstract, \tab/\fig macros, t-SNE caption matches the real 2×2 layout (+
   L/S/E chaining), worst-trace GRL sentence (F4), temperature-scaling advice
   deduplicated. Bib — He-2016 → @inproceedings + address/month, 2020/21 online
   venues marked. Build — Tectonic on `d:\filip\tools\tectonic\` (outside repo;
   `iftex` guard in template.tex keeps pdflatex/Overleaf byte-compatible, T1
   fontenc under xetex → real Times metrics): **exactly 6 pages**, zero
   undefined refs/citations, both figures render. **Forest plot EXCLUDED** (no
   slack inside 6 pages; `tab:test` CI column carries the intervals). Stray
   `report/results.tex.tmp.*` removed. NOTE: do not build in-place in `report/`
   — the output name `template.pdf` would clobber the course template PDF.
   Remaining: presentation. Code freeze 2026-07-28 (deadline 2026-07-30).

## Blockers / open decisions

- **RESOLVED 2026-07-21 — `C1_sharplike` earns a §0.7 test row (17th).** Team
  decision taken after seeing val (0.7384), admissible (val ≠ test; committed to
  report an expected-confirmatory result, no cherry-pick incentive). Registered
  pre-register-AND-commit-to-report; §0.7 list 16 → 17, dated CHANGELOG amendment,
