...
7. **Il test set si valuta una volta sola, alla fine, solo con il checkpoint selezionato su val.** Nessuna eval intermedia sul test, per nessuno stream, per nessun motivo. La harness logga ogni invocazione sul test. **Anche l'eval di C0 (procedura repo SHARP, §2.1) passa dal medesimo logger** tramite wrapper: nessuna via d'accesso al test fuori dall'audit trail. **Nota v5.2:** le righe trasduttive pre-registrate (AdaBN/T3A, §9) usano dati di test **non etichettati** DENTRO la medesima e unica sessione, passano dallo stesso logger e si confrontano con la riga C1 liscia; la lista delle righe è stata congelata alla team call del 2026-07-17 e **non si estende a sessione aperta**. **Lista congelata (16 righe — emendata 2026-07-19 (13, C3-ft), 2026-07-20 (16, C1-aug), 2026-07-21 (17, C1_sharplike) e 2026-07-22 (16, drop C1_aug_s43 per L6), sempre a sessione NON ancora aperta):** C0; C1; C1_s43; C2; C2_s43; C1-lin; C2-lin; C3 (epoch40, selezione fase B); **C3-ft (fine-tuning CE dell'encoder C3, ricetta pre-registrata in `configs/c3_ft.yaml` + CHANGELOG 2026-07-19)**; C1+AdaBN; C1+T3A; C1+AdaBN+T3A; C1 S6-out (rotazione E2′); **C1_aug; C1_s6out_aug (braccio augmentation mirata, profilo `ce_amp` §3, pre-registrato in `configs/c1_ce_aug.yaml` + CHANGELOG 2026-07-20; gemello s43 `C1_aug_s43` eliminato per L6, CHANGELOG 2026-07-22 — confronti appaiati con le righe C1/C1 S6-out esistenti)**; **C1_sharplike (ablation di backbone: rete SHARP-like nella ricetta C1 su p2_lab, pre-registrata in `configs/c1_sharplike.yaml` + CHANGELOG 2026-07-21 — riga di test decisa 2026-07-21 dopo aver visto il val, impegno-a-riportare, esito atteso confermativo)**. Le righe per-antenna di appendice (§6, ablation 1) sono aggregazioni diverse delle stesse invocazioni, non righe aggiuntive. Il readiness assert del notebook 05 verifica questa lista hard-coded (implementazione nello step pre-freeze, con cross-review, §10.4).
...
- **Profilo aggiuntivo `ce_amp` (braccio C1-aug — team call 2026-07-20, pre-registrato in `splits/CHANGELOG.md`):** identico alla colonna CE salvo il **solo canale amplitude** — amplitude scaling **s ~ U(0.6, 1.5), p=0.8**. La tabella sopra resta **immutata**: i profili `ce`/`supcon_view` congelati sono byte-identici (ogni run archiviata riproduce attraverso di essi) e il profilo nuovo è **additivo** in `augment.py`, selezionato per run dalla chiave di config `train.augment_profile` (default `"ce"`, solo percorso CE — le viste SupCon restano pinnate a `supcon_view`, assert in `train.py`). Motivazione del lever (STATUS 2026-07-20, review approfondita): l'attenuazione globale è l'unica manipolazione coerente col μ-Doppler, label-safe e in-scope che modella la componente stanza/attenuazione dello shift cross-ambiente; time-warp respinto per fisica (in μ-Doppler tempo e velocità sono accoppiati), velocity-warp per label-safety (riga sopra: la velocità separa walking/running — stessa ragione del flip vietato). Un solo lever concettuale (range + p del canale amplitude) per attribuzione pulita; usato solo dalle run `C1_aug`/`C1_s6out_aug` (§0.7, §8.4; il config `c1_ce_aug_s43.yaml` dichiara lo stesso profilo ma la run è stata eliminata, L6 2026-07-22). Questo NON riapre l'ablation augmentation eliminata in §6: è un braccio a ipotesi pre-registrata e disegno appaiato, non una griglia esplorativa.
...
**Nota 2026-07-20 (team call C1-aug):** il braccio `C1_aug` (§3 profilo `ce_amp`, §0.7, §10.3) **non** riapre l'ablation augmentation eliminata qui — quella era una griglia esplorativa senza ipotesi, esclusa per budget (ragione poi superata dal budget liberato da C4, ma il merito non fu mai argomentato). Il braccio è una singola variante mirata con ipotesi pre-registrata, disegno appaiato a seed fisso contro le baseline esistenti e replica cross-rotazione; il set §3 core resta senza ablation.
...
**Emendamento v5.2 (cfr. `splits/CHANGELOG.md`):** la riga C4 non si esegue; le estensioni pre-registrate E1′ (2 run ≈ 4.6 h) + E2′ (1 run ≈ 2.5 h) ≈ 7.1 h — dello stesso ordine del budget liberato da C4 (proiezione di gate 6.87 h) e molto sotto l'inviluppo estensioni pre-v5.2 (E1+E2+E3: 15–35 h). Nessun'altra run si aggiunge alla tabella. **Emendamento 2026-07-19 (team call, cfr. CHANGELOG):** aggiunta l'unica run C3-ft ≈ 2 h (fine-tuning CE dell'encoder C3 da `C3/epoch40.ckpt`, seed 42) — totale estensioni ≈ 9.1 h, ancora sotto l'inviluppo pre-v5.2; con questa la tabella è definitivamente chiusa. **Emendamento 2026-07-20 (team call C1-aug, cfr. CHANGELOG):** la chiusura dichiarata alla riga precedente è **riaperta da una decisione di team esplicita** (mai in silenzio): +3 run CE del braccio augmentation mirata — `C1_aug` (P2-lab, s42), `C1_aug_s43` (P2-lab, s43), `C1_s6out_aug` (P2-living, s42) — ≈ 3 × 2.3 ≈ 6.9 h; totale estensioni ≈ 16 h, dentro l'inviluppo pre-v5.2 (15–35 h). Nessuna run di baseline si ripete: i confronti sono appaiati alle run C1/C1_s43/C1_s6out esistenti. **Emendamento 2026-07-22 (team call L6, cfr. CHANGELOG):** eliminato `C1_aug_s43` (gemello seed-43 su P2-lab) — braccio aug 3 → 2 run (`C1_aug` s42 + `C1_s6out_aug` s42), −2.3 h → totale estensioni ≈ 13.7 h. Il confronto aug resta appaiato alle sole run C1/C1_s6out esistenti; `configs/c1_ce_aug_s43.yaml` e il suo runner restano nel repo, non lanciati.
...
4. **C1-aug — braccio augmentation mirata (team call 2026-07-20, cfr. CHANGELOG):** 2 run ≈ 4.6 h — `C1_aug` s42 (P2-lab) e `C1_s6out_aug` s42 (P2-living) (il gemello seed-43 `C1_aug_s43` è stato eliminato per L6, CHANGELOG 2026-07-22). Profilo `ce_amp` (§3, additivo), split e μ/σ congelati riusati, baseline esistenti riusate (confronto appaiato a seed fisso: init e ordine dei batch identici al gemello non aumentato — solo la trasformazione differisce). Priorità del disegno: replica cross-rotazione > secondo seed — il secondo seed (`C1_aug_s43`) è stato poi eliminato (L6, 2026-07-22): il confronto appaiato controlla già l'init e il gemello riusava lo stesso test S7. Ipotesi pre-registrata nel CHANGELOG; due righe di test in §0.7; probe/diagnostiche NON previste su questo braccio (end-to-end soltanto — coerente con l'asimmetria dichiarata E1′).

### 10.4 Giorni 13–15: report e presentazione
- Freeze del codice a **deadline −2 giorni**: **deadline 2026-07-30 → freeze 2026-07-28** (v5.2: sostituisce "giorno 12" — i giorni-piano non tracciano più il calendario reale; date anche in STATUS). Il freeze include (v5.2): T3A, l'aggiunta AdaBN alla harness, `viz.plot_embeddings` e l'estensione del template del notebook 05 (righe trasduttive, caching post-AdaBN, assert della lista righe §0.7) — tutto con cross-review. Il report dichiara: seed singolo (o quanti raggiunti), val in-domain, "AR-set invariance", riproduzione parziale C0 (incluse classi e val 20%), CE non pesata, probe su feature non aumentate, asimmetria batch GRL C2 vs C4, nota checkpointing↔BN se usato, no bit-exactness, ogni contingency ed escalation attivata. **Aggiunte v5.2:** C4 chiusa su evidenza senza run; §7 underpowered (evidenza §9 dalle diagnostiche di dominio); probe e diagnostiche solo su seed 42; righe trasduttive con assunzione di deployment dichiarata, iperparametri fissati senza tuning e variante batch di T3A (deviazione dichiarata dal protocollo online del paper); macro-F1 di val a 5 classi effettive vs test a 8 (non scale-comparabili — audit split); fase A a 300 step/epoca vs 400 CE (escalation (a)); E2′ con proprio split e μ/σ. **Dichiarazioni dallo stress test concettuale (2026-07-20, `CONCEPTUAL_STRESS_TEST.md`; formulazione esatta nel doc):** (L0) la chiusura di C4 è triage di budget + prova di incidenza (§3.C di `CONSOLIDATION_REVIEW.md`), NON "GRL provato inutile dal probe" — il probe di dominio dice "non linearmente decodificabile su train visto", non "nessuna dipendenza da dominio rilevante al transfer"; (L1) claim declassato a "transfer verso n=2 sessioni ostili mai viste" (bedroom 5 / living 1 / lab 1), non "domain generalization"; (L2) ogni riga di test riportata con `n_traces` esplicito, macro-F1 non a 4 cifre come misura, bootstrap appaiato trace-level + caveat di molteplicità; (L4) "due famiglie di evidenza" (readout su feature congelate + fine-tune C3-ft), non "sette strumenti indipendenti"; (L5) C0 = ancora di riproduzione, isolata dal path V-B/P2, **niente seed multipli su C0**; (L7) costo-val del GRL come range (≈3.7–10), C1>C2 direzionale, "il GRL destabilizza" declassato (claim di varianza da n=2). L6 (riduzione braccio aug 3→2) e L8 (decomposizione class-coverage nel notebook-06) sono operativi — vedi STATUS e `CONSOLIDATION_REVIEW.md` §6 G12.
