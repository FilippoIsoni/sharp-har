...
4. **C1-aug — braccio augmentation mirata (team call 2026-07-20, cfr. CHANGELOG):** 3 run ≈ 6.9 h — `C1_aug` s42 + `C1_aug_s43` (P2-lab) e `C1_s6out_aug` s42 (P2-living). Profilo `ce_amp` (§3, additivo), split e μ/σ congelati riusati, baseline esistenti riusate (confronto appaiato a seed fisso: init e ordine dei batch identici al gemello non aumentato — solo la trasformazione differisce). Priorità del disegno: replica cross-rotazione > secondo seed (il secondo seed serve alla stabilità del braccio, non al confronto). Ipotesi pre-registrata nel CHANGELOG; tre righe di test in §0.7 (16 totali); probe/diagnostiche NON previste su questo braccio (end-to-end soltanto — coerente con l'asimmetria dichiarata E1′).
